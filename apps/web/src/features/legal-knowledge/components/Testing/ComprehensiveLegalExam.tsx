...
interface ComprehensiveLegalExamProps {
  ...
  examMode?: 'practice' | 'timed' | 'comprehensive';
  timeLimit?: number;
  /** מעמוד ידע משפטי: מדלג על בחירת מצב ומתחיל מבחן (מתוזמן) או תרגול */
  studyMode?: 'exam' | 'practice';
}
...
export const ComprehensiveLegalExam: React.FC<ComprehensiveLegalExamProps> = ({
  ...
  useEffect(() => {
    if (!studyMode || autoStartedRef.current) return;
    autoStartedRef.current = true;
    startExam(studyMode === 'practice' ? 'practice' : 'timed');
  }, [studyMode, startExam]);
  ...
  const currentQuestionLive = examSession?.questions[examSession.currentQuestionIndex];

  useEffect(() => {
    if (!examSession || examSession.examMode !== 'practice' || !currentQuestionLive) return;
    const answeredMc =
      currentQuestionLive.type === 'multiple-choice' && typeof currentAnswer === 'number';
    const answeredTf =
      currentQuestionLive.type === 'true-false' &&
      (currentAnswer === true || currentAnswer === false);
    if (answeredMc || answeredTf) {
      setShowExplanation(true);
    }
  }, [examSession, examSession?.examMode, currentAnswer, currentQuestionLive]);
        ...
        <CardContent>
          ...
          <FormControl component="fieldset" fullWidth sx={{ mt: 3 }}>
            {currentQuestion.type === 'multiple-choice' && currentQuestion.options ? (
              ...
              >
                {currentQuestion.options.map((option, index) => {
                  const practiceReveal =
                    examSession.examMode === 'practice' && typeof currentAnswer === 'number';
                  const isCorrectOpt = index === currentQuestion.correctAnswer;
                  const isChosen = index === currentAnswer;
              ...
              <RadioGroup
                value={
                  currentAnswer === true ? 'true' : currentAnswer === false ? 'false' : ''
                }
              ...
              >
                {(['true', 'false'] as const).map((v) => {
                  ...
                  const practiceReveal =
                    examSession.examMode === 'practice' &&
                    (currentAnswer === true || currentAnswer === false);
                  const isCorrectOpt = currentQuestion.correctAnswer === boolVal;
                  const isChosen = currentAnswer === boolVal;
                  let rowSx: object = { mb: 1, p: 0.5, borderRadius: 1 };
                  if (practiceReveal) {
                    if (isCorrectOpt) {
                      rowSx = {
