...
export const ComprehensiveLegalSystemExam: React.FC<{ studyMode?: 'exam' | 'practice' }> = ({
  studyMode,
}) => {
  const [currentSection, setCurrentSection] = useState<'intro' | 'exam' | 'results'>(() =>
    studyMode !== undefined ? 'exam' : 'intro',
  );
  ...
  const practiceOn = studyMode === 'practice';
  const answeredHere = !!answers[currentQuestion.id];
        ...
        <CardContent sx={{ p: 4 }}>
          <Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
            ...
            <Box display="flex" gap={1} alignItems="center">
              {practiceOn && (
                <Chip label="תרגול" color="success" size="small" variant="outlined" />
              )}
              <Chip 
          ...
          {currentQuestion.type === 'multiple-choice' && (
            ...
            >
              {currentQuestion.options?.map((option) => {
                const reveal = practiceOn && answeredHere;
                const isCorrectOpt = option === currentQuestion.correctAnswer;
                const isChosen = answers[currentQuestion.id] === option;
                let rowSx: object = { mb: 1, p: 0.5, borderRadius: 1 };
                if (reveal) {
                  if (isCorrectOpt) {
                    rowSx = {
                      ...rowSx,
          ...
          {currentQuestion.type === 'true-false' && (
            <RadioGroup
              ...
              onChange={(e) => handleAnswer(currentQuestion.id, e.target.value)}
            >
              {(['נכון', 'שקר'] as const).map((val) => {
                const reveal = practiceOn && answeredHere;
                const isCorrectOpt = val === currentQuestion.correctAnswer;
                const isChosen = answers[currentQuestion.id] === val;
                let rowSx: object = { mb: 1, p: 0.5, borderRadius: 1 };
                if (reveal) {
                  if (isCorrectOpt) {
                    rowSx = {
                ...
                return (
                  <Box key={val} sx={rowSx}>
                    <FormControlLabel
                      value={val}
                      control={<Radio />}
                      label={val === 'נכון' ? 'נכון ✅' : 'שקר ❌'}
                      sx={{ m: 0 }}
                    />
                  </Box>
                );
          ...
          {practiceOn &&
            ...
            (currentQuestion.type === 'multiple-choice' || currentQuestion.type === 'true-false') && (
              <Alert
                severity={
                  answers[currentQuestion.id] === currentQuestion.correctAnswer ? 'success' : 'error'
                }
                sx={{ mt: 2 }}
              >
                ...
                {currentQuestion.emphasis && (
                  <Typography variant="body2" sx={{ mb: 1 }}>
                    <strong>נקודת דגש:</strong> {currentQuestion.emphasis}
                  </Typography>
                )}
                {currentQuestion.legalPrinciples && currentQuestion.legalPrinciples.length > 0 && (
                  <Typography variant="body2" color="text.secondary">
                    <strong>עקרונות:</strong> {currentQuestion.legalPrinciples.join(' · ')}
                  </Typography>
                )}
              </Alert>
            )}
