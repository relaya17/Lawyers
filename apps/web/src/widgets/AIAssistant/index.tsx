...
export const AIAssistant: React.FC<AIAssistantProps> = ({
  ...
  const generateAIResponse = async (userInput: string): Promise<Message> => {
    ...
    if (lowerInput.includes('סימולטור') || lowerInput.includes('simulator')) {
      return {
        id: Date.now().toString(),
        content:
          'תרחישי מעבדה חוזית (שכירות, עבודה, מכר) נמצאים עכשיו תחת «מקורות המשפט» → מבחנים. סימולטור אולם / בית משפט וירטואלי — ב«בית משפט וירטואלי 2».',
        sender: 'ai',
        timestamp: new Date(),
        type: 'text',
        suggestions: ['מעבר למקורות המשפט', 'בית משפט וירטואלי 2', 'מבחני חוזים', 'דיני עבודה'],
      };
    }
    ...
    if (lowerInput.includes('מו״מ') || lowerInput.includes('negotiation')) {
      return {
        id: Date.now().toString(),
        content:
          'חדר המו״מ זמין מעמודי בית המשפט הווירטואלי (1 או 2), לא מהתפריט הכללי. איך תרצה להמשיך?',
        sender: 'ai',
        timestamp: new Date(),
        type: 'recommendation',
        suggestions: ['בית משפט וירטואלי 2', 'אסטרטגיות מו״מ', 'ניתוח עמדות'],
      };
    }
  ...
  if (compact && !isOpen) {
    ...
    const verticalInset = vertical === 'bottom' ? 'env(safe-area-inset-bottom)' : 'env(safe-area-inset-top)'
    const horizontalInset = horizontal === 'left' ? 'env(safe-area-inset-left)' : 'env(safe-area-inset-right)'
    const baseOffset = isMobile ? 12 : 16
    const badgeCount = messages.length > 1 ? messages.length - 1 : 0
    const ctaLabel = _t('aiAssistant.cta', { defaultValue: 'עוזר AI' })

    const baseSx = {
      position: 'fixed' as const,
      // Keep the FAB above iOS safe areas and avoid covering bottom content.
      [vertical]: `calc(${baseOffset}px + ${verticalInset})`,
      [horizontal]: `calc(${baseOffset}px + ${horizontalInset})`,
      zIndex: 1000,
      bgcolor: 'transparent',
      color: 'common.white',
      backgroundImage: `linear-gradient(135deg, ${theme.palette.secondary.main} 0%, ${theme.palette.secondary.dark} 100%)`,
      border: `1px solid ${alpha(theme.palette.common.white, 0.18)}`,
      boxShadow: `0 12px 30px ${alpha(theme.palette.common.black, 0.22)}`,
      transition: theme.transitions.create(['transform', 'box-shadow'], { duration: theme.transitions.duration.short }),
      '&:hover': {
        boxShadow: `0 16px 34px ${alpha(theme.palette.common.black, 0.28)}`,
    ...
    // Best UX: on small screens keep it compact (square-rounded icon button). On larger screens show label too.
    if (isMobile) {
      return (
        <Tooltip title={ctaLabel}>
          <Fab
            color="secondary"
            aria-label={ctaLabel}
            size="small"
    ...
    return (
      ...
      >
        <Badge badgeContent={badgeCount} color="error">
          <AutoAwesome sx={{ marginInlineEnd: 1 }} />
        </Badge>
        <Typography variant="body2" sx={{ fontWeight: 900, letterSpacing: 0.2, whiteSpace: 'nowrap' }}>
          {ctaLabel}
        </Typography>
      </Fab>
    )
